...
export class SchedulerDateFormatter extends CalendarDateFormatter {
  ...
  public weekViewTitle({
    ...
    daysInWeek,
    startsWithToday,
  }: SchedulerDateFormatterParams): string {
    ...
    const dateInner: Date =
      startsWithToday || daysInWeek < DAYS_IN_WEEK ? date : dateAdapter.startOfWeek(date, { weekStartsOn });
    ...
    return (
      `${firstDay}` +
      (firstDayMonth !== lastDayMonth || lastDay === firstDay ? ' ' + firstDayMonth : '') +
